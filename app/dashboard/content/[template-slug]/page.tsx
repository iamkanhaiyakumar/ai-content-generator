...
import { analyzeSEO } from "@/utils/seoOptimizer";
...
function CreateNewContent() {
  ...
  const GenerateAIContent = async (formData: any) => {
    ...
    try {
      ...
      const metrics = await analyzeSEO(aiResponse, formData.topic || "");
      setSeoMetrics(metrics);

      console.log("Content SEO score:", metrics.score);
      console.log("SEO recommendations:", metrics.suggestions);
    ...
    <div className="p-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 p-5">
        ...
        <div className="col-span-2">
          <OutputSection aiOutput={aiOutput} seoMetrics={seoMetrics} />
        </div>
      </div>
    </div>
  );
